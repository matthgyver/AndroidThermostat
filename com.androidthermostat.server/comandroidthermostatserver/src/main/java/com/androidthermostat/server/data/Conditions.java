...
public class Conditions {
	...
	@Expose private double outsideTemperature = 100;
	...
	public void init(Context context)
	{
        ...
        weatherTimer.schedule(new WeatherTimerTask(), 2000, 10000);
	...
	public void updateWeather()
	{
		...
		String json = Utils.getUrlContents("http://api.openweathermap.org/data/2.5/weather?id=" + Settings.getCurrent().getOpenWeatherMapStation() + "&APPID=" + Settings.getCurrent().getApiKey() );
		...
		try {
			if (resp.main.temp>0)
			{
				...
				this.weatherForecastUrl = Settings.getCurrent().getForecastUrl();
